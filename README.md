...
<div class="navbar">
  DashBox Cloud
  <div class="help-main-btn" onclick="openHelp()">❓</div>
</div>
...
<div class="container">
 ...
 <div id="loginBox" class="card">
  ...
  <div class="key-info" style="margin-top: 10px; font-size: 12px;">🔑 <b>Key From:</b> itsdonline di channel youtube</div>
 ...
 <div id="chatBox">
  <div id="chatHeader">
    💬 Chat
    <span id="adminChatBtn" onclick="clearChat()">🗑️</span>
  </div>
...
function doLogin(){
 ...
 if(accounts[u]) {
    if(accounts[u] !== p) { alert("❌ Wrong password!"); return; }
 } else {
    accounts[u] = p;
    userFiles[u] = [];
...
function renderFiles(){
 ...
 list.innerHTML="<h4 style='color:#555'>🌐 Public Files</h4>";
 ...
 list.innerHTML+="<h4 style='color:#555'>👤 My Files</h4>";
...
function renderChat(){
 ...
 chatMessages.innerHTML = chatData.map((m,i)=>`
  <div class="message"><b>${m.u}</b>: ${m.t} ${isAdmin ? `<span onclick="deleteChat(${i})" style="color:red; cursor:pointer;">✖</span>` : ``}</div>
 `).join('');
